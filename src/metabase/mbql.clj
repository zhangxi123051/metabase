...
(def Field? (s/pred (partial satisfies? Field) "Is a Field?"))
...
(s/defrecord FKFieldID [fk-field-id   :- su/IntGreaterThanZero
...
(def Table? (s/pred (partial satisfies? Table) "Is a Table?"))
...
(s/defrecord Query [aggregation  :- (s/maybe [Aggregation?])
                    breakout     :- (s/maybe [Field?])
                    fields       :- (s/maybe [Field?])
