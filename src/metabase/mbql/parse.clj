...
(s/defn ^:mbql relative-datetime :- RelativeDatetime
  ...
  ([n    :- s/Int
    unit :- (s/maybe mbql/RelativeDatetimeUnit)]
...
(s/defn ^:mbql count :- CountAggregation
  ([]
   (mbql/strict-map->CountAggregation {}))
  ([field :- mbql/Field?]
...
(def ^:private special-clause-parsers
  {:aggregation  (fn [ags]
                   ...
                   (if-not (s/check [mbql/Aggregation?] ags)
                     ags
                     [ags]))
   :breakout     (partial mapv mbql/->Field)
   :fields       (partial mapv mbql/->Field)
