...
;; Thes provide implementations of `->honeysql` that prevents HoneySQL from converting forms to prepared
...
(defn- field->identifier [{table-id :table_id, :as field}]
  (let [db-id   (db/select-one-field :db_id 'Table :id table-id)
    ...
    (hsql/raw (str \[
                   (str/join \. (cons dataset (field/qualified-name-components field)))
                   \]))))
